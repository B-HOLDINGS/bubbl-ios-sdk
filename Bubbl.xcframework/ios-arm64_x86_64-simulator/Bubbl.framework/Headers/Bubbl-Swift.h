...
#ifndef BUBBL_SWIFT_H
#define BUBBL_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="Bubbl",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
+ (BubblPlugin * _Nonnull)shared SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
...
@property (nonatomic, readonly) UNAuthorizationStatus pushAuthorizationStatus;
...
/// Start foreground location monitoring with dedicated location manager
...
- (void)breadcrumb:(NSString * _Nonnull)msg;
/// Return the log file’s URL so host apps can share / AirDrop.
@property (nonatomic, readonly, copy) NSURL * _Nonnull logFileURL;
/// Delete the current log file (starts a fresh one on next log).
- (void)clear;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
